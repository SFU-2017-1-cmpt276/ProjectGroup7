...
class EatInViewController: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    var recipeIDs: [recipe] = [] //initiate array containing all the recipe objects
    var  searchArray: [ingredient] = [] //array containing all recipe ingredients
    var recipes: [recipe] = [] //inititiate array used to store recipes matched with user ingredients
    var count: Int = 0
    ...
    func search_for_ingredient() -> [recipe]{
        ...
        for recipe in recipes{
            ...
            if ingredients.count != 0{
                recipe.accuracy = round((recipe.Count/Double(ingredients.count))*100)
                if recipe.accuracy > 100{
                    recipe.accuracy = 100
                }
                recipe.Count = 0
