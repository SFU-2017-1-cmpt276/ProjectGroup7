...
class EatInViewController: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        searchArray = parseCSV(name: "recipeIngredients")
    ...
    func search_for_ingredient(array: [ingredient]) -> Int{
        for object in array{
            for input in ingredients{
                if(object.Name.lowercased() == input.lowercased()){
                    let i = Int(object.recipeID)
                    recipeIDs[i] = recipeIDs[i] + 1
                }
            }
        }
        var maxPos = -1
        var index = 0
        for object in recipeIDs{
            if object > maxPos{
                maxPos = index
                index = index + 1
            }
    ...
    func parseCSV(name: String) -> Array<ingredient> {
        ...
        do{
            ...
            for row in rows {
                ...
                let iItemToAdd = ingredient(Name: name, recipeID: ID!)
    ...
    @IBAction func RecipeSearch(_ sender: Any) {
        ...
        if ingredients.count < 5 {
            ...
            let recipeNumber =  search_for_ingredient(array: searchArray)
            print(recipeNumber)
